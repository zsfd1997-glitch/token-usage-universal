...
四条内网痛点，开工前心算一遍；任一命中就**先处理再执行**，优先级：宿主 → 编码 → 路径 → 多端归因。

- **宿主没装 skill 库**（opencode 桌面端 / 自研 CLI / 私有 launcher 常见）→ 手动 bootstrap：[references/intranet-bootstrap.md](references/intranet-bootstrap.md)
- **终端只认 GBK 不认 UTF-8**（Windows `chcp` 返回 `936` / 中文渲染成 `鍘熷緥`）→ 三级降级：[references/gbk-terminal.md](references/gbk-terminal.md)
- **默认路径在本机不成立**（企业机 / 映射盘 / 客户端重装）→ `not_found` 不等于没用量，先探测：[references/path-discovery.md](references/path-discovery.md)
- **同机多端 / IDE 内嵌 / 自定义 base_url**（`opencode.exe` + `opencode-cli.exe` 并存；Trae 里跑 opencode；Trae 连阿里百炼 / 内网网关）→ 归因规则和 ingress 配置：[references/multi-endpoint-scenarios.md](references/multi-endpoint-scenarios.md)
...
- 前置三项自检通过（宿主已接入 / 默认路径或 env 覆盖已就位 / 终端编码确定）
...
**第 3 步：贴面板，补结论。** CLI 返回 ascii-hifi 面板后，先原样放进 fenced code block，再补 1 到 3 句高信号结论，最后一句给可选展开方向。GBK 终端三级降级（Tier 1 `chcp 65001` → Tier 2 `--format json` + skill 端英文重绘 → Tier 3 `--plain-ascii` + 英文标签，禁止中英混排）详见 [references/gbk-terminal.md](references/gbk-terminal.md)。
...
- 禁止假设宿主已经加载过 SKILL.md；内网 opencode / 自研 CLI 常常没装 skill 库，首轮必须先确认或手动 bootstrap
- 禁止在默认路径 `not_found` 时直接得出"今天没用量"的结论；一律先路径探测
- 禁止在 GBK 终端直接贴 UTF-8 面板；未确认终端编码前不要输出中文面板
- 禁止在 GBK 降级场景下中英混排输出（GBK 终端只能稳渲 ASCII）
- 禁止把 `opencode.exe` 和 `opencode-cli.exe` 拆成两条 source 报数，或在 CLI 端返回"0 token"——默认视图是全机聚合，两端共享一条 `opencode` source
- 禁止对 `trae` source 反复 diagnose 期望拿到 exact token——DB 加密，只能走 ingress 劫持 base_url

## 一句话操作口令

先自检（宿主 / 路径 / 编码），再选最小命令，先贴面板再补结论，用户停手就收口。
