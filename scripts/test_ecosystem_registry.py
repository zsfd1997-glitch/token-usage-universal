...
class EcosystemRegistryTests(unittest.TestCase):
    ...
    def test_payload_exposes_scope_and_surface_details(self) -> None:
        ...
        gemini = next(item for item in payload["ecosystems"] if item["ecosystem_id"] == "gemini")
        ...
        self.assertEqual(gemini["surfaces"][0]["maturity"], "exact-ready")
        self.assertEqual(gemini["surfaces"][1]["implemented_source_ids"], ["gemini-cli"])
        self.assertEqual(gemini["surfaces"][1]["maturity"], "exact-ready")
